...
class TeamViewSet(viewsets.ModelViewSet):  # knows basic CRUD operations
    ...
    def join(self, request, pk=None):
        # Deliberately NOT using self.get_object() here - get_queryset() only
        # returns teams the user is ALREADY a member of, which would make it
        # impossible to ever join a new team. Look up across all teams instead.
        team = Team.objects.filter(pk=pk).first()

        if not team:
            return Response(
                {"success": False, "message": "Team not found.", "errors": {}},
                status=status.HTTP_404_NOT_FOUND,
            )

        if Membership.objects.filter(team=team, user=request.user).exists():
            return Response(
                {"success": False, "message": "Already a member of this team.", "errors": {}},
        ...
        return Response(
            {"success": True, "message": "Joined team."},
    ...
    def invite(self, request, pk=None):
        ...
        if not requester_membership or requester_membership.role not in [
            ...
            return Response(
                {"success": False, "message": "You do not have permission to invite members.", "errors": {}},
        ...
        if not created:
            return Response(
                {"success": False, "message": "User is already a member.", "errors": {}},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def change_role(self, request, pk=None, user_id=None):
        ...
        log_activity(
            team,
            request.user,
            f"{request.user.email} changed {target_membership.user.email}'s role to {target_membership.role}.",
        )
